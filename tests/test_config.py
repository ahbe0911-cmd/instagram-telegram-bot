...
def test_legacy_owner_user_id_is_optional(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "dummy-token")
    monkeypatch.setenv("ALLOWED_TELEGRAM_USER_ID", "111111111")

    settings = Settings.from_environment()

    assert settings.allowed_telegram_user_id == 111111111


def test_legacy_owner_user_id_must_be_positive_integer(
...
def test_story_api_key_is_optional(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "dummy-token")
    monkeypatch.setenv("SOCLIP_API_KEY", "dummy-api-key")

    settings = Settings.from_environment()

    assert settings.story_api_configured is True
    assert settings.soclip_api_key == "dummy-api-key"
...
def test_story_session_settings_are_optional(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("INSTAGRAM_SESSIONID", "dummy-session")
    monkeypatch.setenv("INSTAGRAM_CSRFTOKEN", "dummy-csrf")
    monkeypatch.setenv("INSTAGRAM_DS_USER_ID", "1111")

    settings = Settings.from_environment()

    assert settings.story_session_configured is True
    assert settings.instagram_username == "owner"
    assert settings.instagram_sessionid == "dummy-session"
    assert settings.instagram_csrftoken == "dummy-csrf"
    assert settings.instagram_ds_user_id == "1111"
